...
class MCPanoramaSkyBox {
  ...
  private locations: Map<string, number | null | WebGLUniformLocation> = new Map();
  private buffer: WebGLBuffer | null = null

  public constructor(urls: [string, string, string, string, string, string]) {
    this.resources = urls;
  }
  ...
  public createShader(gl: WebGLRenderingContext, source: string, shaderType: number) {
  ...
  public createShaderProgram(gl: WebGLRenderingContext, vssource: string, fssource: string) {
  ...
  public async init() {
    ...
    let promises = []
    let panoramas: HTMLImageElement[] = []
    ...
    for(let i = 0; i < panoramas.length; i++) {
      let target = 0
      if(i === 0) target = GL_TEXTURE_CUBE_MAP_NEGATIVE_Z
      else if(i === 1) target = GL_TEXTURE_CUBE_MAP_NEGATIVE_X
      else if(i === 2) target = GL_TEXTURE_CUBE_MAP_POSITIVE_Z
      else if(i === 3) target = GL_TEXTURE_CUBE_MAP_POSITIVE_X
      else if(i === 4) target = GL_TEXTURE_CUBE_MAP_POSITIVE_Y
      else if(i === 5) target = GL_TEXTURE_CUBE_MAP_NEGATIVE_Y
  ...
  public render(time: number) {
    ...
    this.gl.enableVertexAttribArray(<number>this.locations.get('positionLocation'));
    this.gl.bindBuffer(GL_ARRAY_BUFFER, this.buffer);
    this.gl.vertexAttribPointer(<number>this.locations.get('positionLocation'), 2, this.gl.FLOAT, false, 0, 0);
    ...
    this.gl.uniformMatrix4fv(<WebGLUniformLocation>this.locations.get('viewDirectionProjectionInverseLocation'), false, viewM.store());
    this.gl.uniform1i(<WebGLUniformLocation>this.locations.get('skyboxLocation'), 0);
